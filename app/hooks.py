"""Hook registry for shilljudge extension points.

Local mirror of ``shilljudge_core.hooks`` (thread-helper vendors the foundation
modules rather than importing the library — see database.py / models.py). Keep in
sync with core. The leaderboard route routes thread rows through
``ENRICH_LEADERBOARD`` so premium extensions can append columns; with no handlers
registered every hook is a no-op and the base response is unchanged.
"""

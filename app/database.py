...
DB_PATH = Path(os.environ.get("DB_PATH", str(Path(__file__).parent / "thread_helper.db")))

# Per-contest weights applied to each public metric before the (fixed) scoring formula.
# All default to 1.0, which reproduces the unweighted score exactly.
WEIGHT_COLUMNS = (
...
def upsert_post_data(
    ...
    """Store a post + its public score. ``weights`` (the active contest's metric weights) scales
    each public metric before the fixed formula; absent/all-1.0 weights reproduce the unweighted
    score exactly."""
